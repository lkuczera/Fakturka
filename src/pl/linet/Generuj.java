...
public class Generuj {
	private String
	nazwa = "Biuro Pośrednictwa KREDYTOR",
	nip = "583-187-65-07",
	...
	private String[] confArray = {nazwa,nip,adres,kod,miasto,miejsc,konto};
	...
	public Generuj() {
		try {
			...
			JOptionPane.showMessageDialog(null,"Wystąpił błąd skonfiguruj najpierw Moje Dane w menu konfiguracja"+E,"Błąd",JOptionPane.ERROR_MESSAGE);
		}
	}
	 ...
	 * 
	 * @param k_nazwa - nazwa kupujacego
	 * @param k_adres - adres kupujacego
	 * @param k_nip - nip kupujacego
	 * @param k_kod
	 * @param k_miesc
	 * @param usluga
	 * @param pkwiu
	...
	public void generatePdf(String k_nazwa,String k_adres, String k_nip, String k_kod, String k_miesc,
			 String usluga, String pkwiu, String ilosc, float netto, int vat, String platnosc, String termin, String numer_f,
			 String data,String slownie) {
...
//		 and directs a PDF-stream to a file
		...
		if(System.getProperty("os.name").startsWith("Windows")) 
			 vera = BaseFont.createFont("C:\\Windows\\Fonts\\Arial.ttf", BaseFont.CP1250, BaseFont.EMBEDDED);
		else {
			vera = BaseFont.createFont("/usr/share/fonts/corefonts/arial.ttf", BaseFont.CP1250, BaseFont.EMBEDDED);
		}
...
//		ustawienia pierwszej tabeli 
		...
		table1.addCell(new Paragraph("Faktura VAT           ORGINAŁ/KOPIA", font));
		...
		/* table1 zawiera table i table2
		 * table - tabela z danymi wystawiajacego fakture
		 * table2 - tabela z danymi odbiorcy faktury
		 */
...
//		tabelka płatności w kontener
		...
		PdfPCell pl_cell = new PdfPCell(new Phrase("  numer konta:        "+konto));
		pl_table.getDefaultCell().setHorizontalAlignment(Element.ALIGN_RIGHT);
...
//		zawartość tabelki podatku
		PdfPCell pd_cell = new PdfPCell(new Phrase("Rozliczenie sprzedaży według stawek podatku",font));
		...
		if(vat==22) {
			...
			pd_table.addCell("22%");
			pd_table.addCell("vat musi byc 22 lub 7");
			pd_table.addCell("vat musi byc 22 lub 7");
			pd_table.addCell("musi byc 22 lub 7");
